...
class A1CicloVida : AppCompatActivity() {
    var numero = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_a1_ciclo_vida)
        Log.i("ciclo-vida","onCreate")
        val textViewCicloVida = findViewById<TextView>(
            R.id.txv_ciclo_vida
        )
        textViewCicloVida.text = numero.toString()
        val buttonCicloVida = findViewById<Button>(
            R.id.btn_aumentar_ciclo_vida
        )
        buttonCicloVida.setOnClickListener{aumentarNumero()}
    }
    ...
    override fun onRestoreInstanceState(savedInstanceState: Bundle) {
        ...
        if (numeroRecuperado != null){
            ...
            val tectViewCicloVida = findViewById<TextView>(
                R.id.txv_ciclo_vida
            )
            tectViewCicloVida.text = numero.toString()
        }
